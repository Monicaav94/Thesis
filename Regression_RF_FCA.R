...
load("/gpfs1/work/acostave/FINALRESULTS2/forestmodel.RData")
load("/gpfs1/work/acostave/FINALRESULTS2/traindata.RData")
load("/gpfs1/work/acostave/FINALRESULTS2/testdata.RData")
load("/gpfs1/work/acostave/FINALRESULTS2/fully_cleaned_data.RData")
...
# Load raster layers
  ...
  fca <- raster("/gpfs1/data/satellite/sentinel/germany/forest-condition/v0007-0005/2017/tif-Germany_cog/Forest_condition_v0007-0005_Germany_2017_full_bmean_R20m_cog.tif")
...
save(fully_cleaned_data, file = "/gpfs1/work/acostave/FINALRESULTS2/fully_cleaned_data.RData")
...
png("/gpfs1/work/acostave/correlation_matrix.png", width = 1000, height = 800, res = 150)
...
output_dir <- "/work/acostave/FINALRESULTS2"
trainData_file <- file.path(output_dir, "traindata.RData")
dir_path_td <- dirname(trainData_file)
if (!dir.exists(dir_path_td)) {
  dir.create(dir_path_td, recursive = TRUE)
  print(paste("Created directory for traindata.RData:", dir_path_td))
}
print("traindatacreated")
save(trainData, file=trainData_file)


testData_file <- file.path(output_dir, "testdata.RData")
dir_path_ted <- dirname(testData_file)
if (!dir.exists(dir_path_ted)) {
  dir.create(dir_path_ted, recursive = TRUE)
  print(paste("Created directory for testdata.RData:", dir_path_ted))
}
...
random_forest <- train(
  ...
  num.trees = 700,
  tuneGrid = data.frame(mtry = 3, splitrule = "variance", min.node.size = 1)
)

print(random_forest)

save(random_forest, file = "/gpfs1/work/acostave/FINALRESULTS2/forestmodel.RData")
...
ggsave("semivariogram_residuals.png", plot = p_residuals, width = 8, height = 6, dpi = 300)
print(p_residuals)
ggsave("semivariogram_fca.png", plot = p_fca, width = 8, height = 6, dpi = 300)
print(p_fca)
...
color_palette <- viridis(100, option = "C")  
output_path_map <- "/gpfs1/work/acostave/FINALRESULTS2/BlindFriendly_Residuals_Map.png"

breaks <- seq(min(testData$residuals, na.rm = TRUE),
              max(testData$residuals, na.rm = TRUE),
              length.out = 11)

plot1 <- spplot(testData, "residuals",
                main = list(label = "Spatial Residuals", cex = 1.0, font = 2),  
                col.regions = color_palette,
                at = breaks,
                colorkey = TRUE,  
                pch = 16, cex = 0.2, alpha = 0.5)

print(plot1)

png(filename = output_path_map, width = 1200, height = 900, res = 300)
print(plot1)
dev.off()
...
ggsave("predicted_vs_actual.png", plot = pred_actual_plot, width = 8, height = 6, dpi = 300)
...
ggsave("residuals_vs_fitted.png", plot = residuals_plot, width = 8, height = 6, dpi = 300)
...
ggsave("histogram_residuals.png", plot = hist_residuals_plot, width = 8, height = 6, dpi = 300)
...
ggsave("hist_fca_plot.png", hist_fca_plot, width = 8, height = 6, dpi = 300)
...
ggsave("qq_plot_residuals.png", plot = qq_plot_residuals, width = 8, height = 6, dpi = 300)
...
ggsave("qq_plot_fca.png", plot = qq_plot_fca, width = 8, height = 6, dpi = 300)
...
ggsave("variable_importance.png", plot = importance_plot, width = 8, height = 6, dpi = 300)
